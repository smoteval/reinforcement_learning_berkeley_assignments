...
class PGAgent(BaseAgent):
    ...
    def _discounted_return(self, rewards):
        ...
        total_discounted_return = np.sum(discounted_rewards)
        same_discounted_return_list = [total_discounted_return] * len(rewards)
    
        return same_discounted_return_list
    ...
    def _discounted_cumsum(self, rewards):
        ...
        discounted_rewards = np.array([(self.gamma**t) * rewards[t] for t in range(len(rewards))])
        total_discounted_return = np.sum(discounted_rewards)
        discounted_future_rewards = [total_discounted_return] * len(rewards)

        for t in range(1, len(rewards)):
            discounted_future_rewards[t] -= discounted_rewards[t - 1]
            discounted_future_rewards[t] /= self.gamma**t
